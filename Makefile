...
downgrade: ## Downgrade to args name migration in docker compose
	docker compose run --rm migrator alembic downgrade $(args)
.PHONY: downgrade
...
mount-docker-backup:
	docker cp $(COMPOSE_PROJECT_NAME)-pgbackup:/backups/$(args) ./$(args)
.PHONY: mount-docker-backup

restore:
	docker compose exec bot scripts/postgres/restore $(args)
.PHONY: restore
